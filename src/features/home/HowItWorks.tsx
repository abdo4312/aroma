    ...
    return (
        <section ref={sectionRef} className="reveal">
            <div className="mb-10 text-center">
                ...
                <p className="mx-auto mt-3 max-w-[520px] text-[15px] leading-relaxed text-[#6e4f3a]">
            ...
            <div className="how-timeline relative flex flex-col">
                ...
                <div className="absolute left-7 top-[60px] bottom-[60px] w-[2px] bg-gradient-to-b from-transparent via-[rgba(140,98,57,0.35)] to-transparent md:left-1/2 md:-translate-x-1/2" />

                {steps.map((step, i) => (
                    <div
                        key={step.number}
                        className={`group relative z-[1] pb-8 last:pb-0 pl-16 md:pl-0 md:flex md:items-center ${i % 2 === 1 ? 'md:flex-row-reverse' : ''} reveal delay-${i + 1}`}
                    >
                        {/* Center dot */}
                        <div className="absolute left-0 top-0 z-[5] flex h-14 w-14 items-center justify-center rounded-full border-[3px] border-white/60 bg-gradient-to-br from-[#5f3a26] via-[#8c6239] to-[#d9aa7b] text-xl font-extrabold text-[#fff2e2] shadow-[0_0_0_6px_rgba(140,98,57,0.15),0_8px_24px_-6px_rgba(95,57,38,0.5)] transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-110 group-hover:shadow-[0_0_0_8px_rgba(140,98,57,0.2),0_12px_32px_-6px_rgba(95,57,38,0.6)] md:left-1/2 md:top-1/2 md:-mt-7 md:-translate-x-1/2 after:absolute after:inset-[-8px] after:rounded-full after:border-2 after:border-[rgba(140,98,57,0.2)] after:content-[''] after:animate-[howDotPulse_2.5s_ease-out_infinite]">
                            {step.number}
                        </div>

                        {/* Content */}
                        <div className={`relative w-full md:w-[44%] md:flex-shrink-0 ${i % 2 === 0 ? 'md:text-right md:pr-10' : 'md:text-left md:pl-10'}`}>
                            <span className="mb-2.5 inline-flex items-center gap-1.5 rounded-full border border-[rgba(140,98,57,0.25)] bg-[rgba(140,98,57,0.1)] px-3.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#7a5a46]">
                                {step.labelIcon}
                                {step.label}
                            </span>
                            <h3 className="mb-2 text-[22px] font-bold leading-tight text-[#3f2518]">{step.title}</h3>
                            <p className="mb-3.5 text-sm leading-relaxed text-[#6e4f3a]">{step.description}</p>
                            <div className={`flex flex-wrap gap-2 ${i % 2 === 0 ? 'md:justify-end' : ''}`}>
                                {step.features.map((feat) => (
                                    <span key={feat.text} className="inline-flex items-center gap-1.5 rounded-[10px] border border-white/45 bg-white/30 px-3 py-1.5 text-xs font-semibold text-[#5f3a2a] transition-all duration-300">
                                        <span className="text-[#8C6239]">{feat.icon}</span>
                                        {feat.text}
                                    </span>
                                ))}
                            </div>
                        ...
                        {/* Image */}
                        <div className={`relative mt-4 overflow-hidden rounded-[20px] border border-white/40 aspect-[4/3] shadow-[0_12px_35px_-15px_rgba(72,45,32,0.4)] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:-translate-y-1 group-hover:shadow-[0_20px_50px_-15px_rgba(72,45,32,0.55)] md:mt-0 md:w-[44%] md:flex-shrink-0 ${i % 2 === 0 ? 'md:ml-auto' : 'md:mr-auto'}`}>
                            <img
                                src={step.image}
                                alt={step.title}
                                ...
                                loading="lazy"
                            />
                            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[rgba(30,15,8,0.3)] pointer-events-none" />
