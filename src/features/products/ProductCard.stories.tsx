...
import { Heart, Eye, Plus, Star, Clock } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-neutral-100">
        ...
        {isEquipment && (
          <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-[#5d3a27] text-white text-[10px] font-bold uppercase tracking-[1px]">
            {categoryLabel}
          </div>
        )}
      ...
      <div className="p-6">
        ...
        {isEquipment && product.brewTime && (
          <div className="flex items-center gap-1.5 text-[#6B4423]/60 text-xs mb-3">
        ...
        {isEquipment && product.features && product.features.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {product.features.slice(0, 3).map((f: string) => (
              <span
                key={f}
                className="px-2 py-0.5 bg-[#8C6239]/10 text-[#6B4423] text-[10px] rounded-md border border-[#8C6239]/15"
              >
                {f}
              </span>
            ))}
        ...
        <div
          className={cn(
            'flex items-center justify-between pt-3 border-t border-[#8C6239]/10',
